...
class ApiService {
  ...
  Future<List<HistoryEntry>> getHistory(String address, {int page = 1, int limit = 50}) async {
    ...
    for (final item in data) {
      ...
      entries.add(HistoryEntry(
        txid: txid,
        blockHeight: null, // API v1 doesn't provide block height
        time: timestampStr,
        confirmations: 0, // API v1 doesn't provide confirmations
  ...
  Future<TxStatus> getTxStatus(String txid) async {
    // Use ext/gettx endpoint which is available in Iquidus Explorer
    final uri = Uri.parse('$baseUrl/ext/gettx/$txid');
    final res = await http.get(uri);
    if (res.statusCode != 200) throw Exception('Failed to load TX status');
    final data = _safeJsonDecode(res.body);
    
    // Iquidus ext/gettx returns tx data in 'tx' field
    ...
    return TxStatus(
      ...
      blockHeight: data['blockindex'] as int? ?? tx?['blockindex'] as int?,
      time: timestamp != null 
