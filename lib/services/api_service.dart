...
class ApiService {
  ...
  // 4. Send Transaction - Using /api/broadcast endpoint
  Future<String> sendRawTransaction(String rawHex) async {
    // Use the new broadcast endpoint provided by admin
    final uri = Uri.parse('$baseUrl/api/broadcast?hex=$rawHex');
    final res = await http.get(uri);
    ...
    if (res.statusCode != 200) {
      throw Exception('Broadcast failed: HTTP ${res.statusCode} - $body');
    }
    
    // Check for "This method is disabled" or similar errors
    if (body.contains('This method is disabled') || 
        body.contains('Not Found') ||
        body.contains('Cannot GET') ||
        body.contains('404')) {
      throw Exception('Broadcast endpoint not available on this explorer. Please contact the explorer admin to enable /api/broadcast endpoint.');
    }
    ...
    try {
      ...
      if (data['error'] != null) {
        throw Exception('Broadcast failed: ${data['error']}');
      }
      final txid = data['txid']?.toString();
      if (txid != null && txid.length == 64 && RegExp(r'^[a-fA-F0-9]+$').hasMatch(txid)) {
        return txid;
      }
      // If txid is not valid, check if body itself is the txid
      if (body.length == 64 && RegExp(r'^[a-fA-F0-9]+$').hasMatch(body)) {
        return body;
      }
      throw Exception('Invalid TXID returned from server: $txid');
    } catch (e) {
      // Not JSON, check if it's a raw TXID (64 hex chars)
      if (body.length == 64 && RegExp(r'^[a-fA-F0-9]+$').hasMatch(body)) {
        return body;
      }
      // If we get here, something is wrong
      throw Exception('Broadcast failed: Invalid response from server - $body');
    }
  }
