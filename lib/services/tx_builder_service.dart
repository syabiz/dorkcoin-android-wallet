...
class TxBuilderService {
  ...
  List<Utxo> _selectUtxos(List<Utxo> utxos, int neededSats) {
    ...
    if (total < neededSats) {
      final available = _coinToSats(utxos.fold<double>(0, (sum, u) => sum + u.amount));
      final needed = neededSats / 100000000;
      final availableCoins = available / 100000000;
      
      String errorMsg = 'Insufficient funds. ';
      errorMsg += 'Needed: ${needed.toStringAsFixed(8)} DORK, ';
      errorMsg += 'Available: ${availableCoins.toStringAsFixed(8)} DORK';
      
      if (skippedCount > 0) {
        errorMsg += ' ($skippedCount UTXOs skipped due to missing script data)';
      }
