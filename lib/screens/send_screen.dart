...
class _SendScreenState extends State<SendScreen> {
  ...
  Future<void> _broadcastTransaction() async {
    ...
    try {
      ...
      try {
        ...
        // If we can't verify, we'll warn the user
        verified = false;
      }
      ...
      setState(() {
        ...
        if (verified) {
          ...
          _successMessage =
              'Transaction broadcast returned TXID, but verification failed.\n\n'
              'TXID: $txid\n\n'
              'WARNING: The transaction may not have been successfully broadcast. Please check the explorer in a few minutes. If not found, your coins have not been spent.';
        }
        _loading = false;
      });
    } catch (e) {
  ...
  Future<void> _openTxInExplorer(String txid) async {
    await Clipboard.setData(
       ClipboardData(text: 'https://explorer.dorkcoin.org/tx/$txid'),
    );
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
