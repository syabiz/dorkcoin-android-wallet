...
class _SendScreenState extends State<SendScreen> {
  ...
  void dispose() {
    ...
    _api.dispose(); // Dispose ElectrumX connection
    super.dispose();
  }
  ...
  Future<void> _broadcastTransaction() async {
    ...
    try {
      ...
      // Verify the transaction was actually broadcast by checking explorer
      // Retry a few times with delay since there may be propagation delay
      bool verified = false;
      String? verificationError;
      for (int attempt = 0; attempt < 3; attempt++) {
        try {
          // Wait for propagation delay on first retry
          if (attempt > 0) {
            await Future.delayed(const Duration(seconds: 2));
          }
          final txStatus = await _api.getTxStatus(txid);
          // Transaction is valid if it has any confirmations or is in mempool
          verified = txStatus.confirmations > 0 || txStatus.inMempool;
          if (verified) break;
        } catch (e) {
      ...
      setState(() {
        ...
        if (verified) {
          ...
          _successMessage =
              ...
              'TXID: $txid\n\n'
              'Note: The transaction was accepted by the network but may take a moment to appear on the explorer. You can verify it at https://explorer.dorkcoin.org/transaction/$txid';
        }
        _loading = false;
      });
    } catch (e) {
  ...
  Future<void> _openTxInExplorer(String txid) async {
    await Clipboard.setData(
       ClipboardData(text: 'https://explorer.dorkcoin.org/transaction/$txid'),
    );
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
