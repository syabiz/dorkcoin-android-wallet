...
class _WalletScreenState extends State<WalletScreen> {
  ...
  Future<double> _fetchDORKUsdPrice() async {
    try {
      final response = await http.get(Uri.parse('https://cexius.com/api/v2/markets/DORK-USDT/tickers'));
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        // Correcting Cexius API mapping for tickers
        // Usually, tickers endpoint returns {"ticker": {"last": "..."}} or directly {"last": "..."}
        final lastPrice = data['last'] ?? (data['ticker'] != null ? data['ticker']['last'] : null);
        if (lastPrice != null) {
          return double.parse(lastPrice.toString());
        }
      }
    } catch (e) {
      debugPrint('Price fetch error: $e');
    }
    throw Exception('Failed to fetch DORK price from Cexius');
  }
